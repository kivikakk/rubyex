...
RubyValue fixnum_multiply(RubyEnvironment &, RubyValue, const std::vector<RubyValue> &);
...
RubyValue fixnum_multiply(RubyEnvironment &_e, RubyValue _self, const std::vector<RubyValue> &_operand)
{
  // XXX: typecheck `operand'
  return RubyValue::from_fixnum(_self.fixnum * _operand[0].fixnum); // XXX bignum
}
