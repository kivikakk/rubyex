...
RubyValue fixnum_inspect(RubyEnvironment &, RubyValue);
...
void RubyNumericEI::init(RubyEnvironment &_e)
{
  ...
  rb_cFixnum->add_method("inspect", RubyMethod::Create(fixnum_inspect));
...
RubyValue fixnum_add(RubyEnvironment &_e, RubyValue _self, const std::vector<RubyValue> &_operand)
{
  ...
  std::cerr << "Fixnum#+: self: " << _self.fixnum << " + operand: " << _operand[0].fixnum << std::endl;
  return RubyValue::from_fixnum(_self.fixnum + _operand[0].fixnum); // XXX bignum
}

RubyValue fixnum_inspect(RubyEnvironment &_e, RubyValue _self)
{
