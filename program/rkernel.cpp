...
void RubyKernelEI::init(RubyEnvironment &_e)
{
  ...
  rb_mKernel->add_method("print", RubyMethod::Create(kernel_print, ARGS_ARBITRARY));
  rb_mKernel->add_method("puts", RubyMethod::Create(kernel_puts, ARGS_ARBITRARY));
  rb_mKernel->add_method("p", RubyMethod::Create(kernel_p, ARGS_ARBITRARY));
